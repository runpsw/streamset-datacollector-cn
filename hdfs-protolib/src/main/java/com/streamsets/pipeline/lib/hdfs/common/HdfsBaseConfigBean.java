...
public abstract class HdfsBaseConfigBean {
  ...
  @ConfigDef(
      ...
      label = "Hadoop FS URI",
      description = "要从HDFS读取的URI，包括HDFS方案和权限，如下所示: <scheme>://<authority>",
      displayPosition = 10,
      group = "HADOOP_FS"
  )
  ...
  @ConfigDef(
      ...
      type = ConfigDef.Type.STRING,
      label = "HDFS用户",
      description = "如果设置，数据收集器将以该用户的身份写入HDFS。数据收集器用户必须在HDFS中配置为代理用户。",
      displayPosition = 20,
      group = "HADOOP_FS"
  )
  ...
  @ConfigDef(
      ...
      label = "Kerberos身份验证",
      defaultValue = "false",
      description = "如果设置，数据收集器将使用Kerberos主体和keytab连接 o HDFS。",
      displayPosition = 30,
      group = "HADOOP_FS"
  )
  ...
  @ConfigDef(
      ...
      defaultValue = "",
      label = "Hadoop FS配置目录",
      description = "带有HDFS配置文件core-site.xml和HDFS-site.xml的SDC资源目录或符号链接",
      displayPosition = 50,
      group = "HADOOP_FS"
  )
  ...
  @ConfigDef(
      ...
      label = "Hadoop FS配置",
      description = "传递给基础Hadoop文件系统的附加Hadoop属性。这些属性优先于通过“Hadoop FS配置目录”属性加载的属性。",
      displayPosition = 60,
      group = "HADOOP_FS"
  )
  @ListBeanModel
