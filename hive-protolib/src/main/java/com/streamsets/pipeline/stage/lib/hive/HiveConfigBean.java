...
public class HiveConfigBean {
  ...
  @ConfigDef(
      ...
      label = "Hadoop配置文件目录",
      description = "加载core-site.xml、hdfs-site.xml和hive-site.xml文件的绝对路径或SDC资源目录下的目录。",
      displayPosition = 60,
      group = "HIVE"
  )
  ...
  @ConfigDef(
      ...
      label = "附加Hadoop配置",
      description = "额外的配置属性。这里的值覆盖从配置文件加载的值。",
      displayPosition = 70,
      group = "HIVE"
  )
  ...
  @ConfigDef(
      ...
      defaultValue = "-1",
      label = "最大缓存数(条)",
      description = "配置用于存储表相关信息的缓存数。使用-1表示可以缓存无限数量的表。",
      displayPosition = 80,
      group = "ADVANCED"
  )
