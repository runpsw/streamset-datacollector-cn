...
public class JdbcFieldColumnMapping {
  ...
  @ConfigDef(
      ...
      defaultValue = "",
      label = "默认值",
      description = "当数据库不返回行时使用的默认值。如果没有设置，在这种情况下，记录将发送给error。",
      displayPosition = 30
  )
  ...
  @ConfigDef(
      ...
      label = "数据类型",
      description = "字段类型。默认情况下，将使用数据库中的列类型。但是如果提供了字段类型，它将覆盖列类型。注意，如果提供了默认值，那么还必须提供字段类型。",
      displayPosition = 40
  )
