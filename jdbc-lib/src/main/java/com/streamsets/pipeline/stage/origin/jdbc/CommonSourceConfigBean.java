...
public final class CommonSourceConfigBean {
  ...
  @ConfigDef(
      ...
      label = "每秒查询",
      description = "此源每秒可运行的查询数。设为零，没有极限。",
      displayPosition = 60,
      group = "JDBC"
  )
  ...
  @ConfigDef(
      ...
      defaultValue = "0",
      label = "SQL错误重试次数",
      description = "发生SQL错误时的重试次数。为了考虑处理暂时性错误（例如：连接问题、死锁等）的可能性，请在此处提供一个正值。达到指定的重试次数后，下一个错误发生时该阶段将失败，管道错误处理行为将接管该阶段。",
      min = 0,
      displayPosition = 160,
      group = "JDBC"
  )
  ...
  @ConfigDef(
      ...
      defaultValue = "0",
      label = "无更多数据事件生成延迟(秒)",
      description = "在生成“无更多数据”事件之前，处理完所有行后要延迟的秒数。如果希望其他事件首先显示在事件流中，则使用。",
      displayPosition = 201,
      min = 0,
      group = "JDBC"
  )
  ...
  @ConfigDef(
      ...
      defaultValue = "false",
      label = "将时间戳转换为字符串",
      description = "使用String类型,而不是将时间戳表示为数据收集器DATETIME类型。",
      displayPosition = 210,
      group = "JDBC"
  )
